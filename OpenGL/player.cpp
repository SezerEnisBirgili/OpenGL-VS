...
bool Player::placeBlock()
{
    ...
    if (!lookingAtBlock(hitBlock)) return false;

    RaycastHit3D rayHit = intersectRayAABB3D(camera.Position, camera.Front, hitBlock);
    if (!rayHit.collided) return false;

    glm::vec3 faceOffset = getFaceOffset(rayHit.face);
    if (faceOffset == glm::vec3(0.0f)) return false;

    glm::vec3 targetPos = hitBlock + faceOffset;

    if (world->isWithinBounds(targetPos) && !world->isBlockSolid(targetPos)) {
...
{
    if (!world) {
        std::cout << "[breakBlock] failed: World is null" << std::endl;
        return false;
    }
    ...
    if (!world->isBlockSolid(hitBlock)) return false;
